...
<!-- CONTENIDO -->
  ...
  <div id="contenedor" class="content-wrapper">
    ...
    <!-- Main content -->
    <section id="micontenedor" class="content">
...
document.write("Hoy es " + dayName[now.getDay()] + ", " + now.getDate() + " de "+ monName[now.getMonth()] +".")
            ...
            <a href="#" class="small-box-footer">Hora y Fceha </a>
          </div>
        </div>
        <!-- ./col -->
      </div>
        ...
        <section class="col-lg-12 connectedSortable">
          ...
          <div class="nav-tabs-custom">
            ...
            <ul class="nav nav-tabs pull-right">
              <li class="pull-left header"><i class="glyphicon glyphicon-book"></i> Investigaciones en Revision</li>
            </ul>
...
ON t.id_investigacion = i.id_investigacion WHERE u.id_usuario = $id_alumno AND i.status_investigacion LIKE 'EN REVISION' ORDER BY i.fecha_investigacion"); 
$pagina->ejecutar();
echo "<table class='table table-hover table-bordered table-striped'>";
echo "<thead><tr class='warning'><th>Folio</th><th>Titulo de la Investigacion</th><th><center>Fecha</center></th><th></th></tr></thead>";
while($rs=$pagina->fetchResultado()){ 
   $folio = $rs['folio'];
   $existe =$objetoNegocios->relacion_inv($id_alumno,$folio);
   $estado = $objetoNegocios->estatus_inv($id_alumno,$folio);
   if($existe > 0){
     if($estado != "ACEPTADA"){
              $r = "<b>Tienes relacion con esta investigacion</b>";
              $btn = '<button onclick="javascript: unirse(\'#micontenedor\',\'darBaja.php\',&#39;'.$folio.'&#39;)" class="btn btn-danger btn-block btn-lg">Darse de baja en la investigación</button>';
          }else{
              ...
              $btn = '<button onclick="unirse(\'#micontenedor\',\'unirse.php\',&#39;'.$folio.'&#39;)"  class="btn btn-info btn-block btn-lg">Unirse a investigación</button>';
              }else{
              $r = "<b>La Investigacion a sido $estado</b>";
              $btn = "";
          }
...
echo "<tr>";
     ...
     echo "<td><center>".$r."<br>".$btn."</center></td>";
...
function cargarConsulta(div, destino){
 ...
 function unirse(div, destino,folio){
 var pagina = ""+destino+"?folio="+folio;
            $(div).load(pagina);
        }
    </script>
